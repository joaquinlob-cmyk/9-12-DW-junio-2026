...
const rotativeImage = document.getElementById('imagen-rotativa')

function nextImage() {
    rotativeImage.style.opacity = '15%'
    setTimeout(() => {
        count++
        if (count >= images.length){
            count = 0
        }
        rotativeImage.src = images[count]
        rotativeImage.style.opacity = '100%'
    }, 600);
}
...
rotativeImage.addEventListener('mouseenter', stop)
rotativeImage.addEventListener('mouseleave', start)
